            ...
            <router-link to="/">
                <div class="topbar-logo-header d-flex align-items-center">
                    <img src="https://i.pinimg.com/736x/78/4d/19/784d19602be7f1d070b6a13001b7fc5c.jpg" class="logo-icon"
                        style="width: 70px;" alt="logo icon">
            ...
            <div class="user-box dropdown">
                <template v-if="user.check">
                    <a class="d-flex align-items-center nav-link dropdown-toggle dropdown-toggle-nocaret" href="#"
                        ...
                        <img :src="getAvatarUrl(user.avatar)" class="user-img" alt="user avatar">
...
export default {
    ...
    mounted() {
        this.user = {
            ...
            avatar: localStorage.getItem("avatar_kh")
        };
        this.fetchBalance();
    ...
    beforeDestroy() {
        ...
        window.removeEventListener("avatar-updated", this.updateAvatar);
    },
    methods: {
        getAvatarUrl(avatar) {
            if (!avatar || avatar === "null" || avatar === "undefined" || avatar === "") {
                return 'https://cdn.kona-blue.com/upload/kona-blue_com/post/images/2024/09/19/465/avatar-trang-1.jpg';
            }
            return `http://127.0.0.1:8000/storage/avatars/${avatar}`;
        },
        updateAvatar() {
            this.user.avatar = localStorage.getItem("avatar_kh");
        },
        async search() {
            if (!this.query.trim()) {
