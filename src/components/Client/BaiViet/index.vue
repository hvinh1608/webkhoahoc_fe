<template>
  <div class="row">
    <div class="col-lg-12 text-center">
      <h2>{{ $t('articles') }} <i class="fa-regular fa-newspaper"></i></h2>
      <p>{{ $t('articles_desc') }}</p>
    </div>
    <hr>
    <div class="col-lg-12">
            ...
            <template v-for="(value, index) in displayedArticles" :key="index">
              <div class="col d-flex">
                <router-link :to="`/chi-tiet-bai-viet/${value.id}`" class="card-link">
                  <div class="card rounded-4 flex-fill" style="height: 360px;">
                    <img :src="value.hinh_anh" class="card-img-top"
                      style="border-top-left-radius: 20px; border-top-right-radius: 20px; height: 200px;"
                      alt="Bài viết">
                    <div class="card-body">
    ...
    <div class="col-lg-12 d-flex justify-content-center mt-3">
      <button class="btn-baiviet cube-baiviet" @click="toggleLimit()">
        ...
        <div class="text">{{ expanded ? $t('collapse') : $t('see_more') }}</div>
      </button>
