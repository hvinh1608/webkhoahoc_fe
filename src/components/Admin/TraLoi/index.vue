        ...
        <div class="accordion-item border-0 shadow-sm">
          <h2 class="accordion-header">
            <button class="accordion-button collapsed fw-bold" type="button" data-bs-toggle="collapse"
              ...
              {{ tenKhoaHoc ? tenKhoaHoc : binhLuans.ten_khoa_hoc_free ? binhLuans.ten_khoa_hoc_free : 'Khóa học không xác định' }}
            </button>
          </h2>
                  ...
                  <ul class="list-unstyled mb-0">
                    <template v-for="traloi in bl.ds_tra_loi" :key="traloi.id">
                      <li>
                        ...
                        <!-- Ô nhập trả lời con -->
  <div v-if="traloi.showReply" class="input-group mt-2 ms-4">
    <input v-model="traloi.tra_loi_con" placeholder="Nhập trả lời..." class="form-control" />
    <button class="btn btn-success" @click="guiTraLoiCon(bl, traloi)">Gửi</button>
  </div>

                        <!-- Đệ quy hiển thị trả lời con nếu có -->
                        <ul v-if="traloi.children && traloi.children.length" class="list-unstyled ms-4 mt-2">
                          <li v-for="child in traloi.children" :key="child.id">
                            <div class="d-flex justify-content-between align-items-start">
                              <div>
                                <i class="bi bi-chat-left-quote text-success me-1"></i>
                                <strong>
                                  {{ child.ten_nguoi_dung }}
                                  <span v-if="child.vai_tro" class="small fw-bold" :class="{
                                    'text-danger': child.vai_tro === 'Admin',
                                    'text-success': child.vai_tro === 'Nhân viên',
                                    'text-primary': child.vai_tro === 'Khách hàng'
                                  }">
                                    ({{ child.vai_tro }})
                                  </span>:
                                </strong>
                              ...
                              <button @click="xoaTraLoi(bl, child.id)" class="btn btn-link text-danger p-0 ms-2"
                                title="Xóa trả lời">
                                <i class="bi bi-trash3-fill fs-6"></i>
                              </button>
                            </div>

                            <!-- Tiếp tục đệ quy nếu còn con nữa (Cấp 3) -->
                            <ul v-if="child.children && child.children.length" class="list-unstyled ms-4 mt-2">
                              <li v-for="grandchild in child.children" :key="grandchild.id">
                                <div class="d-flex justify-content-between align-items-start">
                                  <div>
                                    <i class="bi bi-chat-left-quote text-success me-1"></i>
                                    <strong>
                                      {{ grandchild.ten_nguoi_dung }}
                                      <span v-if="grandchild.vai_tro" class="small fw-bold" :class="{
                                        'text-danger': grandchild.vai_tro === 'Admin',
                                        'text-success': grandchild.vai_tro === 'Nhân viên',
                                        'text-primary': grandchild.vai_tro === 'Khách hàng'
                                      }">
                                        ({{ grandchild.vai_tro }})
                                      </span>:
                                    </strong>
                                  ...
                                  <button @click="xoaTraLoi(bl, grandchild.id)"
                                    class="btn btn-link text-danger p-0 ms-2" title="Xóa trả lời">
                                    <i class="bi bi-trash3-fill fs-6"></i>
                                  </button>
                                </div>

                                <!-- Tiếp tục đệ quy nếu còn con nữa (Cấp 4) -->
                                <ul v-if="grandchild.children && grandchild.children.length"
                                  class="list-unstyled ms-4 mt-2">
                                  <li v-for="greatgrandchild in grandchild.children" :key="greatgrandchild.id">
                                    <div class="d-flex justify-content-between align-items-start">
                                      <div>
                                        <i class="bi bi-chat-left-quote text-success me-1"></i>
                                        <strong>
                                          {{ greatgrandchild.ten_nguoi_dung }}
                                          <span v-if="greatgrandchild.vai_tro" class="small fw-bold" :class="{
                                            'text-danger': greatgrandchild.vai_tro === 'Admin',
                                            'text-success': greatgrandchild.vai_tro === 'Nhân viên',
                                            'text-primary': greatgrandchild.vai_tro === 'Khách hàng'
                                          }">
                                            ({{ greatgrandchild.vai_tro }})
                                          </span>:
                                        </strong>
                                        {{ greatgrandchild.noi_dung }}
                                      </div>
                                      <button @click="xoaTraLoi(bl, greatgrandchild.id)"
                                        class="btn btn-link text-danger p-0 ms-2" title="Xóa trả lời">
                                        <i class="bi bi-trash3-fill fs-6"></i>
                                      </button>
                                    </div>
  ...
  computed: {
    binhLuanTheoKhoaHoc() {
      ...
      this.ds_binh_luan.forEach(bl => {
        const tenKhoaHoc = bl.ten_khoa_hoc || bl.ten_khoa_hoc_free;
        if (!nhom[tenKhoaHoc]) {
          nhom[tenKhoaHoc] = [];
        }
        nhom[tenKhoaHoc].push(bl);
      });
      return nhom;
    }
  },
