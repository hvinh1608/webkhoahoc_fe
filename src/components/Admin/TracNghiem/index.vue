  ...
  <div class="row">
    <div class="col-lg-3">
      <div class="card">
        ...
        <div class="card-body">
          <div class="mb-2">
            <label class="mb-2">Chọn Khóa Học</label>
            <select v-model="create.khoa_hoc_id" class="form-select">
              <option value="">-- Chọn Khóa Học --</option>
              <option :value="v.id" v-for="v in listKhoaHoc" :key="v.id">
                {{ v.ten_khoa_hoc || v.title }} {{ v.is_free ? '(Miễn phí)' : '(Mất phí)' }}
              </option>
            </select>
          </div>
          <div class="mb-2">
          ...
          <form id="updateFormIphone">
            <div class="row">
              <div class="mb-2">
                ...
                <select v-model="edit.khoa_hoc_id" class="form-select">
                  <option value="">-- Chọn Khóa Học --</option>
                  <option :value="v.id" v-for="v in listKhoaHoc" :key="v.id">
                    {{ v.ten_khoa_hoc || v.title }} {{ v.is_free ? '(Miễn phí)' : '(Mất phí)' }}
                  </option>
...
export default {
  ...
  methods: {
    loadKhoaHoc() {
      // Load khóa học có phí
      const khoaHocPromise = axios.get("http://127.0.0.1:8000/api/admin/loai-khoa-hoc/data", {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("key_admin"),
        },
      });

      // Load khóa học miễn phí
      const khoaHocFreePromise = axios.get("http://127.0.0.1:8000/api/admin/khoa-hoc-free/data", {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("key_admin"),
        },
      });
      ...
      // Load cả 2 cùng lúc
      Promise.all([khoaHocPromise, khoaHocFreePromise])
        .then(([resPaid, resFree]) => {
          console.log("Khóa học có phí:", resPaid.data);
          console.log("Khóa học miễn phí:", resFree.data);

          // Lấy dữ liệu từ cả 2 API
          const paidCourses = (resPaid.data.data1 || resPaid.data.data || resPaid.data || []).map(v => ({...v, is_free: 0}));
          const freeCourses = (resFree.data.data1 || resFree.data.data || resFree.data || []).map(v => ({...v, is_free: 1}));

          // Merge lại
          this.listKhoaHoc = [...paidCourses, ...freeCourses];
          console.log("listKhoaHoc sau merge:", this.listKhoaHoc);
        })
        .catch((error) => {
          console.error("Lỗi load khóa học:", error);
          this.$toast.error("Không tải được danh sách khóa học!");
        });
    },
